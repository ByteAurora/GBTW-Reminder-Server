    ...
    while state:
        ...
        for index, element in enumerate(reversed(elements), 1):
            ...
            if item not in before_notice_list:
                ...
                print(sender.send_notify_to_all('공지사항', element.text.strip()))
            ...
            if item not in before_update_list:
                if '업데이트' in item.title:
                    if '완료' in item.title:
                        ...
                        print(sender.send_notify_to_all('업데이트 완료', element.text.strip()))
                    else:
                        print('업데이트 예정: ', element.text.strip())
                        print(sender.send_notify_to_all('신규 업데이트 공지', element.text.strip()))
                    ...
                    if '완료' in item.title:
                        ...
                        print('업데이트 예정: ', element.text.strip())
                        print(sender.send_notify_to_all('정기점검 공지', element.text.strip()))
                elif '임시' in item.title:
                    if '완료' in item.title:
                        print('임시점검 완료: ', element.text.strip())
                        print(sender.send_notify_to_all('임시점검 완료', element.text.strip()))
                    else:
                        print('임시점검 예정: ', element.text.strip())
                        print(sender.send_notify_to_all('긴급 공지: 임시점검', element.text.strip()))
        ...
        for index, element in enumerate(reversed(elements), 1):
            ...
            if item not in before_event_list:
                if '쿠폰' in item.title:
                    ...
                    print(sender.send_notify_to_all('신규 이벤트 공지(쿠폰)', element.text.strip()))
                elif '원스토어' in item.title:
                    print('이벤트(원스토어): ', element.text.strip())
                    print(sender.send_notify_to_all('원스토어 이벤트', element.text.strip()))
                else:
                    print('이벤트: ', element.text.strip())
                    print(sender.send_notify_to_all('신규 이벤트 공지', element.text.strip()))
        ...
        for index, element in enumerate(reversed(elements), 1):
            ...
            if item not in before_end_event_list:
                ...
                print(sender.send_notify_to_all('이벤트 종료', element.text.strip()))
        ...
        for index, element in enumerate(reversed(elements), 1):
            ...
            if item not in before_event_winner_list:
                ...
                print(sender.send_notify_to_all('이벤트 당첨자 발표', element.text.strip()))
